...
from verl.workers.rollout.logprobs import SELECTED_TOKEN_LOGPROBS_KEY, SelectedTokenLogprobs
...
def _single_turn_loop(
    ...
    *,
    response_length: int,
    configured_positions: tuple[int, ...] | None = None,
    enabled: bool = True,
) -> SingleTurnAgentLoop:
    if not enabled:
        selected_token_config = {"token_ids": None, "positions": None}
    else:
        assert token_output.selected_token_logprobs is not None
        selected_token_config = {
            "token_ids": list(token_output.selected_token_logprobs.token_ids),
            "positions": list(configured_positions) if configured_positions is not None else None,
        }
    config = OmegaConf.create(
        {
            "actor_rollout_ref": {
                "rollout": {
                    ...
                    "selected_token_logprobs": selected_token_config,
...
def _agent_output(
    selected_token_logprobs: SelectedTokenLogprobs,
    *,
...
def test_agent_loop_output_as_dict_excludes_raw_selected_token_logprobs():
    payload = _payload(response_ids=(10,), positions=(0,), rows=((-0.1, -0.2),))
    output = _agent_output(payload)

    fields = output.as_dict()

    assert SELECTED_TOKEN_LOGPROBS_KEY not in fields
    assert output.selected_token_logprobs is payload


@pytest.mark.asyncio
async def test_single_turn_disabled_path_does_not_set_payload_field():
    token_output = TokenOutput(token_ids=[10, 11], extra_fields={})
    loop = _single_turn_loop(token_output, response_length=4, enabled=False)

    output = await _run_single_turn(loop)

    assert output.response_ids == [10, 11]
    assert output.selected_token_logprobs is None
    assert SELECTED_TOKEN_LOGPROBS_KEY not in output.model_fields_set


@pytest.mark.asyncio
async def test_single_turn_finalizes_selected_token_logprobs_after_response_truncation():
    token_output = TokenOutput(
        token_ids=[10, 11, 12, 13, 14],
        selected_token_logprobs=_payload(
    ...
    assert output.selected_token_logprobs.response_token_count == 3
    np.testing.assert_array_equal(output.selected_token_logprobs.positions, np.array([0, 2], np.int32))
    assert output.selected_token_logprobs.response_ids == (10, 11, 12)
    np.testing.assert_allclose(
        output.selected_token_logprobs.logprobs,
        np.array([[-0.1, -0.2], [-0.3, -0.4]], np.float32),
    )


@pytest.mark.asyncio
async def test_single_turn_dense_capture_keeps_every_final_position():
    token_output = TokenOutput(
        token_ids=[10, 11, 12, 13, 14],
        selected_token_logprobs=_payload(
            response_ids=(10, 11, 12, 13, 14),
            positions=(0, 1, 2, 3, 4),
            rows=tuple((-0.1 * (row + 1), -0.2 * (row + 1)) for row in range(5)),
        ),
        extra_fields={},
    )
    loop = _single_turn_loop(token_output, response_length=3, configured_positions=None)
    ...
    assert output.selected_token_logprobs is not None
    assert output.selected_token_logprobs.response_ids == (10, 11, 12)
    np.testing.assert_array_equal(output.selected_token_logprobs.positions, np.array([0, 1, 2], np.int32))
    assert output.selected_token_logprobs.logprobs.shape == (3, 2)
...
async def test_single_turn_allows_positions_unavailable_in_short_response():
    ...
    assert output.selected_token_logprobs is not None
    assert output.selected_token_logprobs.response_token_count == 1
    np.testing.assert_array_equal(output.selected_token_logprobs.positions, np.array([0], np.int32))
...
async def test_single_turn_preserves_present_empty_payload_for_short_response():
    token_output = TokenOutput(
        token_ids=[10],
        selected_token_logprobs=_payload(response_ids=(10,), positions=(), rows=()),
        extra_fields={},
    )
...
class _CaptureRemoteMethod:
    ...
    async def remote(self, data):
        ...
        return {
            "reward_score": 1.0,
            "reward_extra_info": {SELECTED_TOKEN_LOGPROBS_KEY: "must not escape the consumer"},
        }
...
async def test_compute_score_hands_off_copy_without_leaking_payload_to_output():
    payload = _payload(response_ids=(10, 11, 12), positions=(0, 2), rows=((-0.1, -0.2), (-0.3, -0.4)))
    output = _agent_output(payload)
    ...
    assert SELECTED_TOKEN_LOGPROBS_KEY in captured_extra_info
    handed_off = captured_extra_info[SELECTED_TOKEN_LOGPROBS_KEY]
    np.testing.assert_array_equal(handed_off["positions"], np.array([0, 2], np.int32))
    assert handed_off["response_ids"] == (10, 11, 12)
    assert handed_off["logprobs_mode"] == "processed_logprobs"
    assert original_extra_info == {"original": "value"}
    assert SELECTED_TOKEN_LOGPROBS_KEY not in output.extra_fields
    assert output.selected_token_logprobs is None
    assert output.reward_score == 1.0
    assert SELECTED_TOKEN_LOGPROBS_KEY not in output.extra_fields["reward_extra_info"]


@pytest.mark.asyncio
async def test_compute_score_rejects_payload_without_streaming_reward_workers():
    payload = _payload(response_ids=(10, 11, 12), positions=(0,), rows=((-0.1, -0.2),))
...
async def test_agent_loop_rejects_non_single_turn_consumer_path():
    worker = object.__new__(AgentLoopWorker)
    worker.rollout_config = SimpleNamespace(selected_token_logprobs=SimpleNamespace(enabled=True))
...
@pytest.mark.asyncio
async def test_fully_async_client_rejects_selected_token_logprobs():
    client = object.__new__(FullyAsyncLLMServerClient)
    client.config = OmegaConf.create(
        {"actor_rollout_ref": {"rollout": {"selected_token_logprobs": {"token_ids": [101], "positions": [0]}}}}
    )
...
def test_fully_async_client_rejects_selected_token_logprobs_at_initialization():
    config = OmegaConf.create(
        {"actor_rollout_ref": {"rollout": {"selected_token_logprobs": {"token_ids": [101], "positions": [0]}}}}
    )
