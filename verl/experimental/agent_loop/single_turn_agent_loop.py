...
from verl.workers.rollout.logprobs import SELECTED_TOKEN_LOGPROBS_KEY, finalize_selected_token_logprobs
...
class SingleTurnAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], priority: int = 0, **kwargs) -> AgentLoopOutput:
        ...
        selected_output_fields: dict[str, Any] = {}
        selected_token_config = self.rollout_config.get("selected_token_logprobs")
        if selected_token_config is not None and selected_token_config.get("token_ids") is not None:
            if token_output.selected_token_logprobs is None:
                raise RuntimeError("Rollout backend did not return selected-token logprobs for an enabled request.")
            selected_output_fields[SELECTED_TOKEN_LOGPROBS_KEY] = finalize_selected_token_logprobs(
                token_output.selected_token_logprobs,
                configured_positions=selected_token_config.get("positions"),
                configured_token_ids=selected_token_config.token_ids,
